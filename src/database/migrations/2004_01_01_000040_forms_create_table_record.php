    ...
    public function up()
    {
        if(!Schema::hasTable('004_403_record'))
        {
            Schema::create('004_403_record', function (Blueprint $table) {
                ...
                $table->foreign('form_403', 'fk01_004_403_record')
                    ->references('id_401')
                    ->on('004_401_form')
                    ->onDelete('cascade')
                    ->onUpdate('cascade');
                $table->foreign('state_403', 'fk02_004_403_record')
                    ->references('id_400')
                    ->on('004_400_state')
                    ->onDelete('restrict')
                    ->onUpdate('cascade');
                $table->foreign('country_403', 'fk03_004_403_record')
                    ->references('id_002')
                    ->on('001_002_country')
                    ->onDelete('restrict')
                    ->onUpdate('cascade');
                $table->foreign('territorial_area_1_403', 'fk04_004_403_record')
                    ->references('id_003')
                    ->on('001_003_territorial_area_1')
                    ->onDelete('restrict')
                    ->onUpdate('cascade');
                $table->foreign('territorial_area_2_403', 'fk05_004_403_record')
                    ->references('id_004')
                    ->on('001_004_territorial_area_2')
                    ->onDelete('restrict')
                    ->onUpdate('cascade');
                $table->foreign('territorial_area_3_403', 'fk06_004_403_record')
                    ->references('id_005')
